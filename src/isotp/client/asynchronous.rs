...
use crate::device::{AsyncCanDevice, CanListener};
...
#[derive(Clone)]
pub struct AsyncCanIsoTp<D, Device, Channel, Frame>
where
    D: AsyncCanDevice<Device= Device, Channel= Channel, Frame= Frame>,
...
unsafe impl<D, Device, Channel, Frame> Send for AsyncCanIsoTp<D, Device, Channel, Frame>
where
    D: AsyncCanDevice<Device= Device, Channel= Channel, Frame= Frame>,
    Channel: Display + Clone + Hash + Eq,
    Frame: CanFrame {}

impl<D, Device, Channel, Frame> AsyncCanIsoTp<D, Device, Channel, Frame>
where
    D: AsyncCanDevice<Device= Device, Channel= Channel, Frame= Frame>,
    Channel: Display + Clone + Hash + Eq,
    Frame: CanFrame, {
    ...
    pub async fn close(&mut self) {
        ...
        log::info!("ISO-TP(CAN async) - closing device");
        self.device.close().await;
    }

    // #[allow(unused_assignments)]
    // async fn write_p2_star(&mut self, channel: &Channel, p2_star: u32) -> Result<(), IsoTpError> {
    //     let mut state = IsoTpState::Error;
    //
    //     while let Some(v) = self.context.state(channel)? {
    //         if v.contains(IsoTpState::ResponsePending) {
    //             log::info!("ISO-TP(CAN async) - response pending, timeout reset to P2*({}ms)", p2_star);
    //             tokio::time::sleep(std::time::Duration::from_millis(p2_star as u64)).await;
    //         }
    //         else {
    //             state = v;
    //             break;
    ...
    // async fn write_p2(&self, channel: &Channel, p2_server: &P2Context) -> Result<(), IsoTpError> {
...
impl<D, Device, Channel, Frame> CanListener for AsyncCanIsoTp<D, Device, Channel, Frame>
where
    D: AsyncCanDevice<Device= Device, Channel= Channel, Frame= Frame>,
    Channel: Display + Clone + Hash + Eq,
    Frame: CanFrame, {
